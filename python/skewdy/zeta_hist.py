...
for cases in range(ncases):
    ax1.plot(time,ave_wke[cases,1],label=ncases_labels[cases]+'('+ncases_labels_short[cases]+')')
ax1.legend(loc='best',fontsize='small')
ax1.set_xlabel('Time (days)')
ax1.set_ylabel('Average WKE')
ax1.set_xlim(0, focus_time)
ax1.set_title('Average WKE in different vorticity bins, $u_0$ = '+u_0+', cm/s')
...
if(plot_hist==1):
    ...
    ax2.hist(zeta,bins=nbins,normed=True,edgecolor='None')
    ax2.set_title('Initial vorticity distribution')
    ...
    plt.tight_layout()
#    plt.show()                                                                                                                                                                        
    plt.savefig('plots/'+run+'/zeta_hist.eps',bbox_inches='tight')
...
if(plot_dist==1):
    ...
    zeta_square = np.flipud(np.reshape(zeta,(hres,hres)))        #Reshapes the array into a 2-d one                                                                                     
    dist_square = np.flipud(np.reshape(dist,(hres,hres)))        #Reshapes the array into a 2-d one                                                                                        
