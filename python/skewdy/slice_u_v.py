...
vmin_zeta = -0.1/divide_by_zeta
vmax_zeta =  0.1/divide_by_zeta

enforce = 1
vmin_wke = -0.1/divide_by_wke
vmax_wke = 0.1/divide_by_wke
...
for iter_day,no_day in enumerate(day_list):
    ...
    path_zeta  = scratch_location+folder+run+'/output/slice'+sliceloc+slice_no_zeta+spaces_ts+str(ts)+'.dat'
    path_wke   = scratch_location+folder+run+'/output/slice'+sliceloc+slice_no_wke+spaces_ts+str(ts)+'.dat'
...
#        ax.get_yaxis().set_ticks([0,222])
            ...
            if(idx==0):
                ...
                if(iter_day==0):
                    ax.set_title(r'$u$ (m/s)',fontsize=20)
                ...
                if(iter_day==0):
                    ax.set_title(r'$v$  (m/s)',fontsize=20)
                cbar = ax.cax.colorbar(im)
                ax.text(-30, 512/2,r'$\leftarrow$ 222 km $\rightarrow$',rotation='vertical',horizontalalignment='center',verticalalignment='center', fontsize=12)
                ax.text(512/2, 512+30,r'$\leftarrow$ 222 km $\rightarrow$',rotation='horizontal',horizontalalignment='center',verticalalignment='center', fontsize=12)

    plt.savefig('plots/'+run+'slice_u_v_'+str(no_day)+'d_'+depth+'m.eps')
#plt.show()
