...
for run_no,run_iter in enumerate(run_list):
    ...
    plt.plot(out[1:,0],out[1:,1],'-'+colors_list[run_no],label=depth_list[run_no])
...
plt.plot(out[:,0],out[:,0]*0,'-k',linewidth=0.5,label=None)
plt.legend(loc='best',fontsize='small')
plt.xlabel('Time (days)')
#plt.ylabel('Correlation')
plt.ylabel(r'$\zeta$-WKE correlation')
#plt.ylabel('Average WKE in SN regions (m$^2$/s$^2$)')
plt.xlim(0,focus_time)
#plt.ylim(-1,2)
#plt.show()                                                                                                                                                                           
plt.savefig('plots/'+run_list[0]+'/WZ_corr_depth.eps',bbox_inches='tight')
