...
for no_day in day_list:
    ...
    path_wke   = scratch_location+folder+run+'/output/slicehtopw1'+spaces_ts+str(ts)+'.dat'
...
#        ax.get_yaxis().set_ticks([0,222])
            ...
            if(idx==0):
                ...
                ax.text(-512/4, 512/2,r'Top surface after '+str(no_day)+' days',rotation='vertical',horizontalalignment='center',verticalalignment='center', fontsize=16)
            else:
                if(enforce==1):
    ...
    plt.savefig('plots/'+run+'slice_wke_vort_'+str(no_day)+'.eps')
#plt.show()
