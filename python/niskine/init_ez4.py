...
color_sn = colormap(0)
color_bl = colormap(80)
color_or = colormap(185)
...
cor = 1.2419e-4  #Adjusting manually because I'm lazy
...
color = color_or
#color = color_bl
ax1.set_xlabel(r'$\zeta_{rms} \,/\,f$', color=color)  # we already handled the x-label with ax1                                                                                          
ax1.grid(color='k', linestyle='-', linewidth=0.1)                                                                         
ax1.tick_params(axis='x', labelcolor=color)
ax1.set_ylim((-focus_depth,0))
#ax1.set_xlim(0,0.2)
ax1.plot(np.sqrt(zz)/cor,z,linewidth=1.5,color=color,label=r'$\zeta$_{RMS}/$f$')
...
ax2.plot(eke,z,color=color_sn,linewidth=1.5,label='EKE')
#ax2.plot(epe[:-1],zu[:-1],color='goldenrod',label='EPE') #Remove the point of unknown EPE                                               
#ax2.plot(epe[:-1],zu[:-1],color='darkgreen',linewidth=2,label='EPE') #Remove the point of unknown EPE 

#ax2.plot(epe[:-1],zu[:-1],color=color_or,linewidth=2, label='EPE') #Remove the point of unknown EPE 
ax2.plot(epe[:-1],zu[:-1],color=color_bl,linewidth=1.5, label='EPE') #Remove the point of unknown EPE 

ax2.plot(wke,z,color=color_sp,linewidth=1.5,label='WKE')
...
if (show==1):
    ...
    plt.savefig('plots/'+run+'/init_ez4.eps',bbox_inches='tight')
