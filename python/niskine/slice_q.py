...
for iter_day,no_day in enumerate(day_list):
    ...
    if os.path.isfile(path_q):
        ...
        g[:,:,0] = q/cor          #Total q/f = (qg + qw)/f
        g[:,:,1] = (q-qg)/cor     #qw/f = (q-qg)/f 
        g[:,:,2] = zf             #zeta/f                (relative vorticity)
        g[:,:,3] = (qg/cor-zf)    #Lpsi/f = (qg-zeta)/f  (stretching)
...
#                ax.text(512/2, 512+30,r'$\leftarrow$ 222 km $\rightarrow$',rotation='horizontal',horizontalalignment='center',verticalalignment='center', fontsize=12)
                ...
                if(depth=='0'):
                    ax.text(-512/4, 512/2,r'Top surface, '+str(no_day)+' day'+plural_list[iter_day]+uw_title,rotation='vertical',horizontalalignment='center',verticalalignment='center', fontsize=14)
                else:
                    ax.text(-512/4, 512/2,r'z = -'+depth+' m, '+str(no_day)+' day'+plural_list[iter_day],rotation='vertical',horizontalalignment='center',verticalalignment='center', fontsize=14)
