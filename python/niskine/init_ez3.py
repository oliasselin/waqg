...
#ax1.set_xlim(0,0.2)
ax1.plot(np.sqrt(zz)/cor,z,color=color,label=r'$\zeta$_{RMS}/$f$')
...
ax2.plot(eke,z,color=color_sn,label='EKE')
ax2.plot(epe[:-1],zu[:-1],color='goldenrod',label='EPE') #Remove the point of unknown EPE                                               
#ax2.plot(epe[:-1],zu[:-1],color='darkgreen',label='EPE') #Remove the point of unknown EPE 
#ax2.plot(epe[:-1],zu[:-1],color=color_or,label='EPE') #Remove the point of unknown EPE 

ax2.plot(wke,z,color=color_sp,label='WKE')
...
if (show==1):
    ...
    plt.savefig('plots/'+run+'/init_ez3.eps',bbox_inches='tight')
