...
run = 'storm5_uw40/'#'wind_uw10_t30/'
uw_factor = 16#(uw/10)^2
...
#        plt.title(r'Horizontally-averaged WE (m/s)$^{2}$')
        ...
        plt.ylabel('Depth (m)')        
        cbar = plt.colorbar(ticks=np.linspace(0,0.005*uw_factor,5+1,endpoint=True), format=ticker.FuncFormatter(fmt))#
...
#        plt.title(r'Horizontally-averaged WPE (m/s)$^{2}$')
        ...
        plt.xlabel('Time (days)')
        plt.ylabel('Depth (m)')
        cbar = plt.colorbar(ticks=np.linspace(0,0.00002*uw_factor,4+1,endpoint=True), format=ticker.FuncFormatter(fmt))#,format='%.1e')
#        cbar.ax.set_ylabel(r'(m/s)$^{2}$')
