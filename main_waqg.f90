PROGRAM main
  ...
  double precision ::   U_test = 1.,V_test=-1.
  ...
  !Initialize a generic qk, BRk and BIk... 
  call init_psi_generic(uk,vk,wk,bk,psik,psir)
  call normalize_trop(uk,vk,wk,bk,psik,qk,wak)
  qk = psik   !psik holds the initial condition
  dqk= (0.D0,0.D0)  
     ...
     do iky=1,ikty
        do ikx=1,iktx
           ...
           if (L(ikx,iky).eq.1) then
              BRk(ikx,iky,izh0) = psik(ikx,iky,izh1)
              BIk(ikx,iky,izh0) = psik(ikx,iky,izh1)
           else
...
! if(mype==0) write(*,*) "First time step"
 ...
 !Replace the nonlinear advective term with a simple transport by a constant field (U_test,V_test)
 do izh0=1,n3h0
    izh1=izh0+1
    do iky=1,ikty
       ky = kya(iky)
       do ikx=1,iktx
          ...
          if (L(ikx,iky).eq.1) then
             nqk(ikx,iky,izh0) = (i*kx*U_test + i*ky*V_test)* qk(ikx,iky,izh1)
            nBRk(ikx,iky,izh0) = (i*kx*U_test + i*ky*V_test)*BRk(ikx,iky,izh0)
            nBIk(ikx,iky,izh0) = (i*kx*U_test + i*ky*V_test)*BIk(ikx,iky,izh0)
          else
...
!************ Compare the numerical and exact solution *************!
...
do izh1=1,izh1
   do iky=1,ikty
      ...
      do ikx=1,iktx
         ...
         if (L(ikx,iky).eq.1) then
            psik(ikx,iky,izh1) = psik(ikx,iky,izh1)*exp( (-i*kx*U_test -i*ky*V_test -nuh*(1.*kh2)**(1.*ilap))*time )
         else
  ...
  do izh0=1,n3h0
     ...
     do ix=1,n1
        do iy=1,n2 
           ...
           error = ABS(psir(ix,iy,izh1)-BIr(ix,iy,izh0))!/ABS(psir(ix,iy,izh1))
...
!     open (unit = 154673, file = "adv-err.dat", access='append', status='old')
     open (unit = 154673, file = "ts-errq.dat")
     write(154673,"(I6,E12.5,E12.5,E12.5)") delt,L1_global/(n1*n2*n3),sqrt(L2_global/(n1*n2*n3)),Li_global
     if(mype==0) write(*,*) "dt= ", delt,"L1=",L1_global/(n1*n2*n3),"L2=",sqrt(L2_global/(n1*n2*n3)),"Li=",Li_global
  end if

 do id_field=7,nfields                                                                                                                                                                  
    call slices(uk,vk,wk,bk,wak,u_rot,ur,vr,wr,br,war,u_rotr,psir,qr-psir,id_field)                          
 end do 
