...
do ix=1,n1d
   ...
   do iy=1,n2d
      ...
      if(ix<=n1) then
         ...
         Aer = a_r*cos(phi) - a_i*sin(phi)   !\tilde(A) times the complex exponential

         if(z1>=0) f1s(ix,iy,iz1)=Aer*cos(a_z*z1)                                                                    !AR
         if(z2>=0) f2s(ix,iy,iz2)=Aer*( N2_scale*a_z*sin(a_z*z2) - a_z*a_z*cos(a_z*z2) )*exp(-N2_scale*(z2-z0))      !BR
         if(z3>=0) f3s(ix,iy,iz3)=a_p*cos(x)*cos(y)*cos(z3)                                                          !psitr
      else
...
END SUBROUTINE generate_fields_stag
    ...
    SUBROUTINE generate_fields_stag2(f1s,nz1,f2s,nz2,f3s,nz3) !generate_fields for staggered fields (like u,v)
...
END SUBROUTINE generate_fields_stag2
...
END SUBROUTINE generate_fields_stag4
    ...
    SUBROUTINE generate_fields_stag5(f1s,nz1,f2s,nz2,f3s,nz3) !generate_fields for staggered fields (like u,v)
...
do ix=1,n1d
   ...
   do iy=1,n2d
      ...
      if(ix<=n1) then
         ...
         if(z1>=0) f1s(ix,iy,iz1)=(a_x*a_x + a_y*a_y)*Aer*cos(a_z*z1)                                                                    !-nabla^2 AR
         if(z2>=0) f2s(ix,iy,iz2)=(a_x*a_x + a_y*a_y)*Aei*cos(a_z*z2)                                                                    !-nabla^2 AI
         if(z3>=0) f3s(ix,iy,iz3)=0.
      else
         if(z1>=0) f1s(ix,iy,iz1)=0.
         if(z2>=0) f2s(ix,iy,iz2)=0.
...
END SUBROUTINE generate_fields_stag5
